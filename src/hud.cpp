...
int aimpoint::init(){
	...
	program = make_program("shaders/aimpoint_vertex_shader.glsl",0, 0, 0, "shaders/aimpoint_fragment_shader.glsl");
	if (!program)
		return 1;
...
int hud::init(){
	...
	tex = load_texture("images/letters.png");

	program = make_program("shaders/hud_vertex_shader.glsl", 0, 0, 0, "shaders/hud_fragment_shader.glsl");
	if(!program)
		return 1;
	v_attrib = glGetAttribLocation(program, "in_vertex");
